...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 5; i++)
            try {
                ResponseEntity<String> responseEntity = sendScreen(true);

                setSleepIntervalFromResponseHeaders(responseEntity.getHeaders());
    ...
    private static void setSleepIntervalFromResponseHeaders(HttpHeaders headers) {
        headers.entrySet().stream()
                .filter(entry -> "sleep_interval".equalsIgnoreCase(entry.getKey()) && !entry.getValue().isEmpty())
                .findAny()
                .ifPresent(entry -> {
                    try {
    ...
    // returns true if we need to send another screen after doing these actions
    private static boolean doActions(List<Action> actions) {
        boolean needToSendAnotherScreen = false;
        ...
        return needToSendAnotherScreen;
    }

    // deleteActions false tells the server not to send the actions this time
    private static ResponseEntity<String> sendScreen(boolean deleteActions) throws Exception {
        ...
        headers.set("delete_actions", String.valueOf(deleteActions));
